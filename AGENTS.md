...
- `.env` - `DISCORD_TOKEN`, `GITHUB_PAT`, `GUILD_ID`, `USER_INTERACTIONS_ENCRYPTION_KEY` (32-byte base64/hex key; required whenever the auto-responder greeter or a persistent classifier is configured - startup fails without it, no fallback), `MESSAGE_LOG_ENCRYPTION_KEY` (a separate 32-byte key required when `logging.enabled`), optional `HEALTH_PORT` (default 9010), `HEALTH_HOST` (default `127.0.0.1` - loopback; set `0.0.0.0` to expose deliberately), `PROPOSAL_API_KEY` (required when `proposals.enabled`), and `FINGERPRINT_HUB_API_KEY` (required when `moderation.image_fingerprint.hub_enabled`). Copy from `template.env`.
...
### Message lifecycle logging (`src/features/messageLogging/`)

Opt-in logging for one configured guild captures human-authored message snapshots before other pipeline features, then records substantive edits, single deletes, and grouped bulk deletes. Bots, webhooks, system messages, moderation-blacklisted channels, logging-specific exclusions, and log destinations are excluded with thread-parent awareness. `messageUpdate` and bulk-delete hooks run through the same isolated feature registry; legacy delete hooks receive per-message fallbacks for bulk events.

`store.ts` uses SQLite/WAL with AES-256-GCM encrypted snapshot and durable-outbox payloads. Snapshot retention defaults to 30 days / 100,000 rows; the outbox is capped at 10,000 rows and retained seven days. `messageLogger.ts` retries Discord delivery with enforced nonces, emits CV2 staff cards plus bounded UTF-8 evidence files, and can best-effort rehost Discord-hosted raster images without writing bytes to disk. Audit-log attribution is deliberately conservative: only one recent matching delete entry names a moderator; internal moderation deletes are marked explicitly; everything else remains author-or-unknown. Operator guide: `docs/MESSAGE_LOGGING.md`.
